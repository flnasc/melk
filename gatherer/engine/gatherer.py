...
def gatherer(keyword, start_date, end_date, scope, sources):
    ...
    for source in sources:
        if source == "new_york_times":
            ...
            nyt = nyt_gatherer.search_nyt(keyword, start_date, end_date, FIELDS)

        if source == "reddit":
            logging.info("Searching Reddit....")
            reddit = reddit_gatherer.search_reddit(keyword, start_date, end_date, FIELDS)
        ...
        if source == "poetry_foundation":
            ...
            poems = poems_gatherer.search_poems(keyword, FIELDS, apiconfig.poetry_dataset_path)

        if source == "twitter":
            logging.info("Searching Twitter....")
            twitter = twitter_gatherer.search_twitter(keyword, start_date, end_date, FIELDS)
